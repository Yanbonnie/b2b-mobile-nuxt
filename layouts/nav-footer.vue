    ...
    <div class="footer-nav">
      <ul>
        ...
        <nuxt-link tag="li" to="/login" v-if="!$store.getters.isAuth">
          <van-icon name="description" :color="navActive === 2 ? '#E3324C' : ''" />去登录
        </nuxt-link>

        <nuxt-link v-else tag="li" to="/order">
          <van-icon name="description" :color="navActive === 2 ? '#E3324C' : ''" />我的订单
        </nuxt-link>
        <nuxt-link v-if="$store.getters.isAuth" tag="li" to="/member">
          <van-icon name="contact" :color="navActive === 3 ? '#E3324C' : ''" />用户中心
        </nuxt-link>
      </ul>
    </div>
  </div>
